...
def matrix_mul(m_a, m_b):
    ...
    Raises:
        TypeError: not a list of lists, or contains non-integer/float elements.
        ValueError: If ma and mb are empty/if the matrices can't be multiplied.

    """
    # Validate m_a
    if not isinstance(m_a, list)
    or not all(isinstance(row, list) for row in m_a):
    ...
    result =
    [[sum(a * b for a, b in zip(row_a, col_b)) for col_b
